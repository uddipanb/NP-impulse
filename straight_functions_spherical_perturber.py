...
EPSREL_plummer=1e-5 #Default: 1e-8
EPSABS=1e-7 #Default: 1e-7
EPSREL=1e-2 #Default: Core: 1e-7, Cusp: 1e-2
maxiter=100
zetamax=1e4 #Default: 1e5
...
def opts1(r,b,rp,perturber_flag,subject_flag):
    if (r>=(b+rp)):
        return {'epsabs': EPSABS, 'epsrel': EPSREL, 'limit': maxiter, 'points' : [(b+rp)/r,abs(b-rp)/r]}
    elif (r<(b+rp) and r>=abs(b-rp)):
        return {'epsabs': EPSABS, 'epsrel': EPSREL, 'limit': maxiter, 'points' : [abs(b-rp)/r]}
    else:
        return {'epsabs': EPSABS, 'epsrel': EPSREL, 'limit': maxiter}
...
def fbound_integrand(phi,theta,r,b,rp,mass_ratio,vp,vcm,R_half_mass,Mtrunc,adcorr_flag,perturber_flag,subject_flag):
    x=r*np.sin(theta)*np.cos(phi)
    y=r*np.sin(theta)*np.sin(phi)
    z=r*np.cos(theta)
    adiabatic_correction_factor=adiabatic_correction(r,b,vp,R_half_mass,Mtrunc,adcorr_flag,subject_flag) 
    v=(2/mass_ratio)*np.sqrt(vx(x,y,z,b,rp,vp,perturber_flag)**2+(vy(x,y,z,b,rp,vp,perturber_flag)-vcm)**2+vz(x,y,z,b,rp,vp,perturber_flag)**2)*np.sqrt(adiabatic_correction_factor)
    vesc=np.sqrt(2*np.abs(Phi(r,subject_flag)))
    return rho(r,subject_flag)*r**2*np.sin(theta)*np.heaviside(vesc-v,1)

def fbound(b,Rcut,rp,mass_ratio,vp,vcm,perturber_flag,subject_flag):
    return (1/Menc(Rcut,subject_flag))*integrate.tplquad(fbound_integrand,epsinit,Rcut,lambda x: epsinit,lambda x: np.pi*(1-epsinit),lambda x,y: epsinit,lambda x,y: 2*np.pi*(1-epsinit),args=(b,rp,mass_ratio,vp,vcm,perturber_flag,subject_flag,),epsabs=EPSABS,epsrel=EPSREL)[0]
...
def fboundtidal_integrand(phi,theta,r,b,rp,mass_ratio,vp,I0_per,I1_per,J0_per,J1_per,R_half_mass,Mtrunc,adcorr_flag,perturber_flag,subject_flag):
    x=r*np.sin(theta)*np.cos(phi)
    y=r*np.sin(theta)*np.sin(phi)
    z=r*np.cos(theta)
    adiabatic_correction_factor=adiabatic_correction(r,b,vp,R_half_mass,Mtrunc,adcorr_flag,subject_flag) 
    v=(2/mass_ratio)*(1/(b**2*vp))*np.sqrt(I0_per**2*x**2+(3*J0_per-J1_per-I0_per)**2*y**2+(2*I0_per-I1_per-3*J0_per+J1_per)**2*z**2)*np.sqrt(adiabatic_correction_factor)
    vesc=np.sqrt(2*np.abs(Phi(r,subject_flag)))
    return rho(r,subject_flag)*r**2*np.sin(theta)*np.heaviside(vesc-v,1)

def fboundtidal(b,Rcut,rp,mass_ratio,vp,R_half_mass,Mtrunc,adcorr_flag,perturber_flag,subject_flag):
    I0_per=I0(b,rp,perturber_flag)
    I1_per=I1(b,rp,perturber_flag)
    J0_per=J0(b,rp,perturber_flag)
    J1_per=J1(b,rp,perturber_flag)
    return (1/Menc(Rcut,subject_flag))*integrate.tplquad(fboundtidal_integrand,epsinit,Rcut,lambda x: epsinit,lambda x: np.pi*(1-epsinit),lambda x,y: epsinit,lambda x,y: 2*np.pi*(1-epsinit),args=(b,rp,mass_ratio,vp,I0_per,I1_per,J0_per,J1_per,R_half_mass,Mtrunc,adcorr_flag,perturber_flag,subject_flag,),epsabs=EPSABS,epsrel=EPSREL)[0]
